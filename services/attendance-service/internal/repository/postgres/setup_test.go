...
func ensureMigrations(t *testing.T, db *pgxpool.Pool) {
	...
	// Note: When running tests, the working directory is the package directory.
	...
	for _, file := range files {
		if !file.IsDir() && filepath.Ext(file.Name()) == ".sql" && len(file.Name()) > 7 && file.Name()[len(file.Name())-7:] == ".up.sql" {
			...
			if err != nil {
				// Ignore errors like "relation already exists" for idempotency if simple
				// But ideally we should handle this better. For now, we assume clean DB or safe migrations.
